...
public class AuthService {
	...
	public ResponseEntity<JwtResponse> authenticateUser(LoginForm loginRequest) {
		...
		return ResponseEntity
				.ok(new JwtResponse(jwt, userDetails.getUsername(), userDetails.getAuthorities(), currentUser.getId()));
	}
	...
	public ResponseEntity<ResponseMessage> registerUser(SignUpForm signUpRequest) throws IOException {
		...
		user.setLogo(file);
		Optional<User> recommendedBy = Optional
				.ofNullable(userRepository.findByUsername(signUpRequest.getRecommander())).orElse(null);
